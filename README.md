...
> Note: Strapi only auto-discovers packages whose names start with `strapi-plugin-` (or `@scope/strapi-plugin-*`). Because this package uses `@devx-labs/strapi-preview`, the explicit `resolve` entry above is required.
